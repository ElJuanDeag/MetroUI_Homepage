...
import type { ClientMessage, CreateRoomRequest, ServerMessage } from "../shared/messages.js"
...
const serveStatic = async (requestPath: string) => {
...
const httpServer = createServer(async (req, res) => {
  try {
    if (req.method === "POST" && req.url === "/api/rooms") {
      ...
      const payload = JSON.parse(Buffer.concat(chunks).toString("utf8")) as CreateRoomRequest
...
wsServer.on("connection", (socket, request) => {
  ...
  socket.on("message", (raw) => {
    try {
      ...
      if (!roomId || !playerId) throw new Error("Join a room first.")
