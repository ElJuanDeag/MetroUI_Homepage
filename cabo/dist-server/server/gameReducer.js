...
export function createRoomState(roomId, hostName, hostToken, settings) {
    const now = Date.now();
    return {
        roomId,
        phase: "lobby",
...
const playerById = (room, playerId) => {
    const player = room.players.find((entry) => entry.id === playerId);
    if (!player)
        throw new Error("Player not found.");
    return player;
};
const replacePlayer = (room, nextPlayer) => {
    room.players = room.players.map((player) => (player.id === nextPlayer.id ? nextPlayer : player));
};
const nextPlayerId = (room, currentId) => {
...
const drawCard = (room) => {
    if (room.deck.length === 0) {
        ...
        if (!topDiscard || room.discard.length === 0)
...
const visibleCard = (player, index) => {
    const card = player.cards[index];
    if (!card)
        throw new Error("Card index out of range.");
    return card;
};
const swapCards = (a, aIndex, b, bIndex) => {
...
const resolvePowerCard = (room, playerId, first, second, swap) => {
    const pending = room.pendingPower;
    if (!pending || pending.actorId !== playerId)
        throw new Error("No power pending.");
    const actor = playerById(room, playerId);
    if (pending.type === "peek-self") {
        if (!first || first.playerId !== playerId)
            throw new Error("Choose one of your own cards to peek.");
        const card = visibleCard(actor, first.cardIndex);
        if (!actor.knownToSelf.includes(card.id))
            actor.knownToSelf.push(card.id);
        room.pendingPower = undefined;
    ...
    if (pending.type === "peek-opponent") {
        if (!first)
            throw new Error("Choose a card to peek.");
    ...
    if (pending.type === "blind-swap") {
        if (!first || !second)
            throw new Error("Choose two cards to swap.");
    ...
    if (!first)
        throw new Error("Choose a card for the king action.");
    const firstPlayer = playerById(room, first.playerId);
    const firstCard = visibleCard(firstPlayer, first.cardIndex);
    if (!pending.hasPeeked) {
        if (!actor.knownToSelf.includes(firstCard.id))
            actor.knownToSelf.push(firstCard.id);
...
export function reduceGame(room, action) {
    ...
    if (action.type === "START_GAME") {
        ...
        if (room.players.length < 2 || room.players.length > 4)
            throw new Error("Cabo requires 2 to 4 players.");
    ...
    if (action.type === "DRAW_FROM_DECK") {
        ...
        if (room.pendingDraw || room.pendingPower)
    ...
    if (action.type === "SWAP_DRAWN_CARD") {
        ...
        if (!draw)
            throw new Error("No drawn card to swap.");
        const player = playerById(room, action.playerId);
    ...
    if (action.type === "DISCARD_DRAWN_CARD") {
        ...
        if (draw.mustSwap)
            throw new Error("Discard pile draws must be swapped.");
        room.discard.push(draw.card);
        room.pendingDraw = undefined;
        const power = draw.card.power;
        if (power) {
            room.pendingPower = { type: power, actorId: action.playerId };
    ...
    if (action.type === "CALL_CABO") {
        assertTurn(room, action.playerId);
        if (room.pendingDraw || room.pendingPower)
            throw new Error("Finish your active turn before calling Cabo.");
    ...
    if (action.type === "SLAP_DISCARD") {
        if (!room.settings.slapEnabled)
            throw new Error("Slap is disabled for this room.");
        const player = playerById(room, action.playerId);
